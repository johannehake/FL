...
pub_house <- "EDU" # change to the publishing house code of interest
pub_data <- fl %>% 
...
EDU_plot <- ggplot(year_counts, aes(x = fl_year, y = num_books)) +
  geom_line(color = "darkgrey", size = 1) +
  xlab("Year") +
  ylab("Books Published") +
  ggtitle(paste("Books published by EDU, before/after Crimea")) +
  theme_tufte() + 
